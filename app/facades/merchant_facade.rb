class MerchantFacade
  class << self
    def all_merchants_helper(params) #TODO: test?
      return Merchant.all_merchants() if params[:page].nil? && params[:per_page].nil?
      return Merchant.all_merchants(params[:page].to_i, params[:per_page].to_i) if params[:page].present? && params[:per_page].present?
